...
public class EmployeeMapper {
    ...
    public EmployeeResponse map(Employee employee) {
        return new EmployeeResponse(employee.getName(), employee.getEmail(), employee.getSex());
    }

    public List<EmployeeResponse> map(List<Employee> employees) {
        List<EmployeeResponse> response = new ArrayList<>();
        for(Employee employee : employees){
            response.add(new EmployeeResponse(employee.getName(), employee.getEmail(), employee.getSex()));
        }
        return response;
    }
}
