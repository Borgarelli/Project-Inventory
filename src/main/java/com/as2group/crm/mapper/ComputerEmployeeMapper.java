...
public class ComputerEmployeeMapper {
	...
	public ComputerEmployeeResponse map(ComputerEmployee computerEmployee) {
		EmployeeResponse employee = this.employeeMapper.map(computerEmployee.getEmployee());
		
		return new ComputerEmployeeResponse(computerEmployee.getId_comp_empl(), employee, computerEmployee.getReceived(), computerEmployee.getReceived());
	}
